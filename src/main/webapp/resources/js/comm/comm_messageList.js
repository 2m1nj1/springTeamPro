...
//	}); // end of .click()
	...
	function loadMessages() {
			$.ajax({
				url: '/common/comm_getMessageList.do',  // 모든 메시지 목록을 가져오는 AJAX 호출
				type: 'GET',
				success: function(response) {
					 //alert("성공");
								
					 var messages = response;  // 서버에서 받은 JSON 데이터를 그대로 사용

					// 받은 쪽지와 보낸 쪽지 목록을 구분하여 테이블에 추가
					var $receivedTbody = $('#tab_msg_recive .tbody_board');
					var $sentTbody = $('#tab_msg_send .tbody_board');

					$receivedTbody.empty();  // 받은 쪽지 목록 초기화
					$sentTbody.empty();      // 보낸 쪽지 목록 초기화

					
					// 받은 쪽지 목록을 추가
					message.forEach(function(message) {
						// 받은 쪽지 처리 (받은 사람과 일치하는 메시지)
						if (message.msg_reciver === user_account.user_no) {
							var row = '<tr data_id=1"' + message.msg_no + '">'
									+ '<td><input type="checkbox" class="checkbox"></td>'
									+ '<td>' + message.msg_reciver + '</td>'
									+ '<td><a href="getMessage.do?msg_no=' + message.msg_no + '">' + message.msg_Title + '</a></td>'
